...
public class Grafo {
    ...
    public void MatrizDistancias(){
            ...
            for(int j = 0; j < vertices.size(); j++){
                ...
                if (ady == true){
                    for (int z = 0; z < conexiones.size(); z++){
                        if ((conexiones.get(z).inicio.equals(vertices.get(i)) && conexiones.get(z).fin.equals(vertices.get(j))) || (conexiones.get(z).fin.equals(vertices.get(i)) && conexiones.get(z).inicio.equals(vertices.get(j)))){
        ...
        for (int i = 0; i < vertices.size(); i++) {
            for(int j = 0; j < vertices.size(); j++){
                Recorrido[i][j] = vertices.get(j).nb;
            }
        }
    }
   ...
   public String FloydWarshall(vertices inicio, vertices fin){
        ...
        String Ruta = vertices.get(inicioPos).nb;
        ...
        while (finalizado == false){
            if (Recorrido[inicioPos][finPos].equals(inicio.nb) || Recorrido[inicioPos][finPos].equals(fin.nb)){
                ...
                for (int i = 0; i < vertices.size(); i++){
                    if (vertices.get(i).nb.equalsIgnoreCase(tmp)){
                        inicioPos = i;
                    }
                }
            }
        ...
        if (CostoAsociado >= Integer.MAX_VALUE) {
            Ruta = "No existe ruta de " + inicio.nb + " hasta " + fin.nb;
        }
        return Ruta;
    }
